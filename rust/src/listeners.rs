...
pub extern fn Java_me_kyleclemens_spongejni_rust_generated_RustyListener_joinReceived(env: *mut JNIEnv, this: jobject, event: jobject) {
  ...
  let user = unsafe { entity_living_player_User::from(env, player.object) };
  let receiver = unsafe { text_channel_MessageReceiver::from(env, player.object) };
  receiver.send_message(text_Text::of_rust(env, &format!("Welcome, {}!", user.get_name().into_rust_string(env))));
}
